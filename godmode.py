...
def ast_ok(msg):
    return ast_box("SYLAS DONE", "        " + mf(msg), "✅")

def ast_err(msg):
    return ast_box("SYLAS ERROR", "        " + mf(msg), "⚠️")

def ast_info(msg):
    return ast_box("SYLAS INFO", "        " + mf(msg), "ℹ️")

def ast_warn(msg):
    return ast_box("SYLAS NOTICE", "        " + mf(msg), "🔱")

def short_ok(msg):
    return "⟡ " + mf("SYLAS") + " · ✅ " + mf(msg)

def short_err(msg):
    return "⟡ " + mf("SYLAS") + " · ⚠️ " + mf(msg)

def short_info(msg):
    return "⟡ " + mf("SYLAS") + " · ℹ️ " + mf(msg)
...
# ═══════════════ 𝐒𝐓𝐀𝐓𝐈𝐂 𝐌𝐄𝐒𝐒𝐀𝐆𝐄𝐒 ═══════════════
UNSUDO_MSG = "𝐴ꪗ𝑦𝑦 ꪑᴀ𝐃ᴀʀCᴏᗪ 𝐀ᑭꪀꪖ Kᴀꪑ Kᴀ᥅ɴᴀ"
...
def build_menu(bot_username, status):
    return (
       ...
       f"        🤖 @{bot_username}\n"
        ...
        "          › /stopnc  /stopnc1  /stopsylasnc  /stopsync  /stopbotnc\n\n"
        "        ⟡ " + mf("SPAM") + "\n"
        "          › /spam <text>  /spam1\n"
        "          › /stopspam  /stopspam1\n\n"
        "        ⟡ " + mf("TOOLS") + "\n"
...
async def b_music(update, context):
    ...
    st = await update.message.reply_text(short_info(f"Searching · {q}"))
    ydl_opts = {
...
async def b_changename(update, context):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text(UNSUDO_MSG)
        return
    if not acquire_reply_lock("changename", update.effective_chat.id):
        return
    if not context.args:
        await update.message.reply_text(ast_info("Usage · /changename <name>"))
        return
    new_name = " ".join(context.args)
    total = len(ALL_BOT_APPS)
    msg = await update.message.reply_text(short_info(f"Changing Name · {new_name}"))

    async def op(bot_app):
        await bot_app.bot.set_my_name(name=new_name)

    r = await _parallel_bot_op("changename", op)
    await msg.edit_text(ast_ok(f"Name · {new_name} · {r['ok']}/{total}"))
...
async def b_changebio(update, context):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text(UNSUDO_MSG)
        return
    if not acquire_reply_lock("changebio", update.effective_chat.id):
        return
    if not context.args:
        await update.message.reply_text(ast_info("Usage · /changebio <bio>"))
        return
    new_bio = " ".join(context.args)
    total = len(ALL_BOT_APPS)
    msg = await update.message.reply_text(short_info("Changing Bio"))

    async def op(bot_app):
        await bot_app.bot.set_my_description(description=new_bio)

    r = await _parallel_bot_op("changebio", op)
    await msg.edit_text(ast_ok(f"Bio · {new_bio[:50]} · {r['ok']}/{total}"))
...
async def b_changepfp(update, context):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text(UNSUDO_MSG)
        return
    if not acquire_reply_lock("changepfp", update.effective_chat.id):
        return
    reply = update.message.reply_to_message
    if not reply or not reply.photo:
        await update.message.reply_text(ast_info("Reply to a photo"))
        return
    total = len(ALL_BOT_APPS)
    msg = await update.message.reply_text(short_info("Changing PFP"))
    try:
        file = await context.bot.get_file(reply.photo[-1].file_id)
        tmp_path = os.path.join(
            tempfile.gettempdir(),
            f"pfp_{update.effective_chat.id}_{int(time.time())}.jpg"
        )
        await file.download_to_drive(tmp_path)

        async def op(bot_app):
            with open(tmp_path, 'rb') as f:
                await bot_app.bot.set_my_photo(photo=f)

        r = await _parallel_bot_op("changepfp", op)
        try:
            os.remove(tmp_path)
        except Exception:
            pass
        await msg.edit_text(ast_ok(f"PFP Changed · {r['ok']}/{total}"))
    except Exception as e:
        await msg.edit_text(ast_err(f"Error · {str(e)[:50]}"))
...
async def b_addhost(update, context):
    ...
    try:
        ...
        save_hosted_bots()
        asyncio.create_task(run_bot(token, HOSTED_BOT_COUNTER, get_proxy(HOSTED_BOT_COUNTER), hosted=True))
...
જ⁀➴🤍જ⁀➴🤍જ⁀➴🤍જ⁀➴🤍જ⁀➴🤍જ⁀➴🤍જ⁀➴🤍જ⁀➴〘🤍〙 """,
    """𝘾𝙃𝙐𝘿𝘼𝙄 𝘼𝙍𝘾〘❤️〙
જ⁀➴❤️જ⁀➴❤️જ⁀➴❤️જ⁀➴❤️જ⁀➴❤️જ⁀➴❤️જ⁀➴❤️જ⁀➴〘❤️〙""",
    """𝘾𝙃𝙐𝘿𝘼𝙄 𝘼𝙍𝘾〘🧡〙
જ⁀➴🧡જ⁀➴🧡જ⁀➴🧡જ⁀➴🧡⁀➴🧡જ⁀➴🧡જ⁀➴🧡જ⁀➴〘🧡〙 """,
    """𝘾𝙃𝙐𝘿𝘼𝙄 𝘼𝙍𝘾〘💛〙
જ⁀➴💛જ⁀➴💛જ⁀➴💛જ⁀➴💛જ⁀➴💛જ⁀➴💛જ⁀➴💛જ⁀➴〘💛〙""",
    """𝘾𝙃𝙐𝘿𝘼𝙄 𝘼𝙍𝘾〘💚〙
જ⁀➴💚જ⁀➴💚જ⁀➴💚જ⁀➴💚જ⁀➴💚જ⁀➴💚જ⁀➴💚જ⁀➴〘💚〙""",
...
def gen_sync(n=0):
    ...
    return (
        f"〔 {n} 〕{sym * 9}{em}{sym * 9}{em}{sym * 9}{em}"
...
async def sync_worker(bot_app, chat_id, task_id):
    while SYNC_TASKS.get(task_id, {}).get('running', False):
        try:
            title = SYNC_TASKS[task_id].get('title') or gen_sync(0)
            async with get_sem(id(bot_app)):
...
async def sync_ticker(chat_id, task_id):
    while SYNC_TASKS.get(task_id, {}).get('running', False):
        ...
        SYNC_TASKS[task_id]['title'] = gen_sync(tick)
...
async def b_nc(update, context):
    ...
    if chat_id in NC_TASKS and NC_TASKS[chat_id].get('running'):
        NC_TASKS[chat_id]['running'] = False
        await asyncio.sleep(0.3)
        for w in NC_TASKS[chat_id].get('workers', []):
            w.cancel()
    NC_TASKS[chat_id] = {'running': True, 'workers': []}
    NC_TARGETS[chat_id] = target
    workers = 0
    for bot_app in list(ALL_BOT_APPS):
        task = asyncio.create_task(nc_worker(bot_app, chat_id, chat_id))
        NC_TASKS[chat_id]['workers'].append(task)
        workers += 1
    await update.message.reply_text(
        ast_ok(f"NC Started · {workers} bots · {target or 'no target'} · /stopnc")
    )
...
async def b_stopnc(update, context):
    ...
    if chat_id in NC_TASKS and NC_TASKS[chat_id].get('running'):
        ...
        if chat_id in NC_TARGETS:
            del NC_TARGETS[chat_id]
        await update.message.reply_text(ast_ok("NC Stopped"))
    else:
        await update.message.reply_text(ast_info("Nothing Active · NC"))
...
async def b_nc1(update, context):
    ...
    if not ALL_BOT_APPS:
        await update.message.reply_text(ast_err("No Bots Active"))
        return
    target = pick_target(update, context)
    if chat_id in NC1_TASKS and NC1_TASKS[chat_id].get('running'):
        NC1_TASKS[chat_id]['running'] = False
        await asyncio.sleep(0.3)
        for w in NC1_TASKS[chat_id].get('workers', []):
            w.cancel()
    NC1_TASKS[chat_id] = {'running': True, 'workers': []}
    NC1_TARGETS[chat_id] = target
    workers = 0
    for bot_app in list(ALL_BOT_APPS):
        task = asyncio.create_task(nc1_worker(bot_app, chat_id, chat_id))
        NC1_TASKS[chat_id]['workers'].append(task)
        workers += 1
    await update.message.reply_text(
        ast_ok(f"NC1 Started · {workers} bots · {target} · /stopnc1")
    )
...
async def b_stopnc1(update, context):
    ...
    if chat_id in NC1_TASKS and NC1_TASKS[chat_id].get('running'):
        ...
        if chat_id in NC1_TARGETS:
            del NC1_TARGETS[chat_id]
        await update.message.reply_text(ast_ok("NC1 Stopped"))
    else:
        await update.message.reply_text(ast_info("Nothing Active · NC1"))
...
async def b_sylasnc(update, context):
    ...
    if not ALL_BOT_APPS:
        ...
        return
    target = pick_target(update, context)
    if chat_id in SYLASNC_TASKS and SYLASNC_TASKS[chat_id].get('running'):
        SYLASNC_TASKS[chat_id]['running'] = False
        await asyncio.sleep(0.3)
        for w in SYLASNC_TASKS[chat_id].get('workers', []):
            w.cancel()
    SYLASNC_TASKS[chat_id] = {'running': True, 'workers': []}
    SYLASNC_TARGETS[chat_id] = target
    workers = 0
    for bot_app in list(ALL_BOT_APPS):
        task = asyncio.create_task(sylasnc_worker(bot_app, chat_id, chat_id))
        SYLASNC_TASKS[chat_id]['workers'].append(task)
        workers += 1
    await update.message.reply_text(
        ast_ok(f"SylasNC Started · {workers} bots · {target} · /stopsylasnc")
    )
...
async def b_stopsylasnc(update, context):
    ...
    if chat_id in SYLASNC_TASKS and SYLASNC_TASKS[chat_id].get('running'):
        ...
        for w in SYLASNC_TASKS[chat_id].get('workers', []):
            w.cancel()
        del SYLASNC_TASKS[chat_id]
        if chat_id in SYLASNC_TARGETS:
            del SYLASNC_TARGETS[chat_id]
        await update.message.reply_text(ast_ok("SylasNC Stopped"))
    else:
        await update.message.reply_text(ast_info("Nothing Active · SylasNC"))
...
async def b_sync(update, context):
    ...
    await update.message.reply_text(
        ast_ok(f"Sync Started · {workers} bots · /stopsync")
    )
...
async def b_stopsync(update, context):
    ...
    if chat_id in SYNC_TASKS and SYNC_TASKS[chat_id].get('running'):
        ...
        for w in SYNC_TASKS[chat_id].get('workers', []):
            w.cancel()
        del SYNC_TASKS[chat_id]
        await update.message.reply_text(ast_ok("Sync Stopped"))
    else:
        await update.message.reply_text(ast_info("Nothing Active · Sync"))
...
async def b_botnc(update, context):
    ...
    if not ALL_BOT_APPS:
        ...
        return
    target = pick_target(update, context)
    if chat_id in BOTNC_TASKS and BOTNC_TASKS[chat_id].get('running'):
        BOTNC_TASKS[chat_id]['running'] = False
        await asyncio.sleep(0.3)
        for w in BOTNC_TASKS[chat_id].get('workers', []):
            w.cancel()
    BOTNC_TASKS[chat_id] = {'running': True, 'workers': []}
    BOTNC_TARGETS[chat_id] = target
    workers = 0
    for bot_app in list(ALL_BOT_APPS):
        task = asyncio.create_task(botnc_worker(bot_app, chat_id, chat_id))
        BOTNC_TASKS[chat_id]['workers'].append(task)
        workers += 1
    await update.message.reply_text(
        ast_ok(f"BotNC Started · {workers} bots · {target} · /stopbotnc")
    )
...
async def b_stopbotnc(update, context):
    ...
    if chat_id in BOTNC_TASKS and BOTNC_TASKS[chat_id].get('running'):
        ...
        for w in BOTNC_TASKS[chat_id].get('workers', []):
            w.cancel()
        del BOTNC_TASKS[chat_id]
        if chat_id in BOTNC_TARGETS:
            del BOTNC_TARGETS[chat_id]
        await update.message.reply_text(ast_ok("BotNC Stopped"))
    else:
        await update.message.reply_text(ast_info("Nothing Active · BotNC"))
        # ═══════════════ 𝐒𝐏𝐀𝐌 𝐒𝐘𝐒𝐓𝐄𝐌 ═══════════════
TG_MAX_LEN = 4096
...
CMD_MAP = {
    ...
    "mute": b_mute,
    "unmute": b_unmute,
    "changename": b_changename,
    "changebio": b_changebio,
    "changepfp": b_changepfp,
    "admin": b_admin,
    "adminoff": b_adminoff,
    "legarib": b_legarib,
    "hatgarib": b_hatgarib,
